...
function setMarkers(data) {
  for (let i = 0; i < data.address.length; i++) {
      let curr_add = data.address[i]
      let street = new Date(curr_quake.properties["time"])
      let city = curr_quake.properties["mag"]
      let state = curr_quake.properties["place"]
      let zip = curr_quake.geometry.coordinates[2];
